...
1. **Builder**（GitHub Actions，每日）：`scripts/generate-feed.mjs`
   ...
   - 头部资管研究（`channel: "research"`，公开 feed，非爬墙）：Apollo Academy（Torsten Sløk）、Oaktree（Howard Marks Memos）走博客 RSS；Bridgewater「Research & Insights」走官方 YouTube 播放列表 Atom feed；Carlyle「Insights & Indicators」（Jason Thomas）走官方播客 RSS（Castos）。统一在 `scripts/lib/rss.mjs` 解析（含 `media:description` / `itunes:summary`）。
